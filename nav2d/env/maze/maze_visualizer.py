...
class MazePlot:
    ...
    """
    def __init__(self, width, height, maze_object, path=None,
                 maze_line_color="purple", maze_name="untitled"):
        self.width = width
        self.height = height
    ...
    def save_as_image(self):
        plt.savefig(f"{self.maze_name}.png")
        return
    ...
    def get_edge_locations(self):
        '''
        get x and y data for matplotlib 2D lines that are
        legal edges and should not be colored in on plot
        '''
        edge_data = []
        for edge in sorted(self.edges):
            ...
            if e1[0] == e2[0]:
                ...
                if e1[1] > e2[1]:
                    # if the y value of the first coordinate is greater, need a left edge  # noqa: E501
                    edge_data.append([[e1[1], e1[1]], [e1[0] + 1, e1[0]]])
                else:
                    # need a right edge
                    edge_data.append([[e1[1] + 1, e1[1] + 1],
                                      [e1[0], e1[0] + 1]])
            if e1[1] == e2[1]:
        ...
        for i in range(self.height):
            for j in range(self.width):
                ...
                if top_edge in edge_data:
                    ...
                    self.ax.plot([j, j + 1], [i, i],
                                 color=self.maze_line_color,
                                 linewidth=self.maze_line_width)
